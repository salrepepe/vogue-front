import React from "react";
import { Typography, Box } from "@mui/material";
const Contacts = () => {
  return (
    <div>
      <Box
        sx={{
          display: "flex",
          justifyContent: "space-between",
          flexDirection: { xs: "column", md: "row" },
          rowGap: "30px",
          "& .div": {
            display: "flex",
            alignItems: "center",
            columnGap: 1,
          },
        }}
      >
        <div className="div">
          <svg
            width="23"
            height="17"
            viewBox="0 0 23 17"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
            <path
              d="M1.56814 16.9999H5.22722V8.24477L0 4.38232V15.4549C0 16.3098 0.703111 16.9999 1.56814 16.9999Z"
              fill="black"
            />
            <path
              d="M17.7727 16.9991H21.4319C22.2996 16.9991 23 16.3063 23 15.4541V4.38135L17.7727 8.24391"
              fill="black"
            />
            <path
              d="M17.7727 1.54948V8.24458L23 4.38214V2.32186C23 0.411257 20.7863 -0.678023 19.2364 0.467909"
              fill="#505050"
            />
            <path
              d="M5.22729 8.24404V1.54905L11.5 6.18388L17.7727 1.54883V8.24393L11.5 12.8791"
              fill="#8D8D8D"
            />
            <path
              d="M0 2.32186V4.38191L5.22722 8.24447V1.54948L3.76361 0.467909C2.21111 -0.678023 0 0.411257 0 2.32186Z"
              fill="#505050"
            />
          </svg>
          <Typography component="span" variant="h5">
            voguecontact@gmail.com
          </Typography>
        </div>
        <div className="div">
          <svg
            width="20"
            height="18"
            viewBox="0 0 20 18"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
            <path
              fill-rule="evenodd"
              clip-rule="evenodd"
              d="M17.9283 0.120468C18.1742 0.0138 18.4434 -0.0229891 18.7078 0.0139295C18.9722 0.0508481 19.2221 0.160125 19.4316 0.330386C19.6411 0.500647 19.8025 0.725668 19.899 0.982028C19.9955 1.23839 20.0235 1.51671 19.9802 1.78804L17.7234 15.8967C17.5044 17.2577 16.0556 18.0381 14.8446 17.3602C13.8316 16.7931 12.327 15.9193 10.9737 15.0076C10.297 14.5512 8.22427 13.0898 8.47901 12.0498C8.69793 11.1607 12.1807 7.81939 14.1709 5.83287C14.952 5.05241 14.5958 4.60219 13.6734 5.32008C11.3827 7.10252 7.70483 9.81309 6.48884 10.5761C5.41614 11.2489 4.8569 11.3637 4.1882 11.2489C2.96823 11.0397 1.83682 10.7156 0.913377 10.3207C-0.33446 9.78745 -0.27376 8.01937 0.912382 7.50454L17.9283 0.120468Z"
              fill="black"
            />
          </svg>

          <Typography component="span" variant="h5">
            voguecontact@gmail.com
          </Typography>
        </div>
        <div className="div">
          <svg
            width="20"
            height="20"
            viewBox="0 0 20 20"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
            <path
              d="M10 0C15.523 0 20 4.477 20 10C20 15.523 15.523 20 10 20C8.23277 20.003 6.4966 19.5353 4.97002 18.645L0.00401546 20L1.35602 15.032C0.464974 13.5049 -0.00308163 11.768 1.52687e-05 10C1.52687e-05 4.477 4.47702 0 10 0ZM6.59202 5.3L6.39201 5.308C6.26271 5.31691 6.13636 5.35087 6.02002 5.408C5.91159 5.46951 5.81258 5.5463 5.72602 5.636C5.60602 5.749 5.53802 5.847 5.46502 5.942C5.09514 6.4229 4.89599 7.01331 4.89902 7.62C4.90102 8.11 5.02902 8.587 5.22902 9.033C5.63802 9.935 6.31102 10.89 7.19902 11.775C7.41302 11.988 7.62302 12.202 7.84902 12.401C8.95245 13.3724 10.2673 14.073 11.689 14.447L12.257 14.534C12.442 14.544 12.627 14.53 12.813 14.521C13.1042 14.5056 13.3885 14.4268 13.646 14.29C13.7769 14.2223 13.9047 14.1489 14.029 14.07C14.029 14.07 14.0714 14.0413 14.154 13.98C14.289 13.88 14.372 13.809 14.484 13.692C14.568 13.6053 14.638 13.5047 14.694 13.39C14.772 13.227 14.85 12.916 14.882 12.657C14.906 12.459 14.899 12.351 14.896 12.284C14.892 12.177 14.803 12.066 14.706 12.019L14.124 11.758C14.124 11.758 13.254 11.379 12.722 11.137C12.6663 11.1128 12.6067 11.0989 12.546 11.096C12.4776 11.0888 12.4084 11.0965 12.3432 11.1184C12.278 11.1403 12.2182 11.176 12.168 11.223C12.163 11.221 12.096 11.278 11.373 12.154C11.3315 12.2098 11.2744 12.2519 11.2088 12.2751C11.1433 12.2982 11.0723 12.3013 11.005 12.284C10.9398 12.2666 10.876 12.2446 10.814 12.218C10.69 12.166 10.647 12.146 10.562 12.11C9.98789 11.8599 9.45646 11.5215 8.98702 11.107C8.86102 10.997 8.74402 10.877 8.62402 10.761C8.23063 10.3842 7.88777 9.95799 7.60401 9.493L7.54501 9.398C7.50328 9.3338 7.46905 9.26501 7.44302 9.193C7.40502 9.046 7.50402 8.928 7.50402 8.928C7.50402 8.928 7.74702 8.662 7.86002 8.518C7.97002 8.378 8.06301 8.242 8.12302 8.145C8.24102 7.955 8.27802 7.76 8.21602 7.609C7.93602 6.925 7.64668 6.24467 7.34802 5.568C7.28902 5.434 7.11402 5.338 6.95502 5.319C6.90102 5.31233 6.84701 5.307 6.79302 5.303C6.65874 5.2953 6.52411 5.29664 6.39002 5.307L6.59202 5.3Z"
              fill="black"
            />
          </svg>

          <Typography component="span" variant="h5">
            +996 559 43 33 21{" "}
          </Typography>
        </div>
        <div className="div">
          <svg
            width="14"
            height="20"
            viewBox="0 0 14 20"
            fill="none"
            xmlns="http://www.w3.org/2000/svg"
          >
            <path
              d="M2.36188 0.43881L3.43888 0.0948098C4.44688 -0.22719 5.52488 0.29381 5.95688 1.31181L6.81688 3.33981C7.19188 4.22281 6.98388 5.26181 6.30288 5.90781L4.40988 7.70581C4.52655 8.78181 4.88788 9.84081 5.49388 10.8828C6.07016 11.8907 6.84232 12.773 7.76488 13.4778L10.0409 12.7178C10.9029 12.4308 11.8419 12.7618 12.3709 13.5388L13.6029 15.3488C14.2189 16.2528 14.1079 17.4988 13.3449 18.2648L12.5269 19.0858C11.7129 19.9028 10.5509 20.1998 9.47488 19.8638C6.93622 19.0718 4.60188 16.7208 2.47188 12.8108C0.339218 8.89414 -0.413449 5.57148 0.213884 2.84281C0.477884 1.69481 1.29588 0.77981 2.36388 0.43881"
              fill="black"
            />
          </svg>

          <Typography component="span" variant="h5">
            +996 559 43 33 21{" "}
          </Typography>
        </div>
      </Box>
    </div>
  );
};

export default Contacts;
